    ...
    <div x-data="{ isOpen: false }" @keydown.escape.window="isOpen = false" class="min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap justify-between items-center gap-4 mb-8">
                    <div class="flex space-x-2">
                        <button class="px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-700/50 rounded-lg text-sm text-gray-300 transition-colors">
                            All
                        </button>
                        <button class="px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-700/50 rounded-lg text-sm text-gray-300 transition-colors">
                            Income
                    ...
                    <button @click="isOpen = true" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center transition-colors">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                    <div class="bg-gray-800/30 backdrop-blur-sm rounded-xl border border-gray-700/50 p-5">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-xs font-medium px-2 py-1 bg-green-500/10 text-green-400 rounded-lg">+12.5%</span>
                        </div>
                    ...
                    <div class="bg-gray-800/30 backdrop-blur-sm rounded-xl border border-gray-700/50 p-5">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-xs font-medium px-2 py-1 bg-red-500/10 text-red-400 rounded-lg">+8.4%</span>
                        </div>
                    ...
                    <div class="bg-gray-800/30 backdrop-blur-sm rounded-xl border border-gray-700/50 p-5">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-xs font-medium px-2 py-1 bg-blue-500/10 text-blue-400 rounded-lg">+4.1%</span>
                        </div>
                ...
                <div class="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b border-gray-700/50">
                        ...
                        <div class="flex items-center space-x-2 text-sm text-gray-400">
                            <span>View:</span>
                            <select class="bg-gray-700/50 border-none rounded-md text-sm text-gray-300 py-1 pl-2 pr-6 focus:ring-1 focus:ring-gray-500">
                                <option>All</option>
                        ...
                        <table class="min-w-full divide-y divide-gray-700/50">
                            <thead>
                                <tr class="bg-gray-800/40">
                                    ...
                                    <th class="px-5 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-gray-800/20 divide-y divide-gray-700/50">
                                @foreach ($depenses as $depense)
                                <tr class="hover:bg-gray-700/20 transition-colors">
                                    <td class="px-5 py-3.5 text-sm font-medium text-gray-300">{{ $depense->depense_date ?? $depense->created_at->format('M d, Y') }}</td>
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center">
                                            <div class="h-8 w-8 rounded-md flex items-center justify-center bg-red-500/10 border border-red-500/20 mr-3">
                                                <svg class="w-4 h-4 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                                                </svg>
                                            </div>
                                            <div>
                                                <div class="text-sm font-medium text-white">{{ $depense->category->name }}</div>
                                                <div class="text-xs text-gray-400">{{ $depense->category->description }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-3.5 text-sm text-gray-300">Expense</td>
                                    <td class="px-5 py-3.5 text-sm font-medium text-red-400">-{{ number_format($depense->price, 2) }} DH</td>
                                    <td class="px-5 py-3.5">
                                        <span class="px-2 py-1 text-xs font-medium bg-green-500/10 text-green-400 rounded-lg">
                                            {{ $depense->status ?? 'Done' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-3.5 text-right">
                                        <div class="flex justify-end space-x-2">
                                            <button class="p-1.5 rounded hover:bg-gray-700/50 text-gray-400 hover:text-white transition-colors">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </button>
                                            @if ($depense->type === 'recurrentes')
                                            <form action="{{route('depense.destroy' , $depense->id)}}" method="GET">
                                                <button type="submit" class="p-1.5 rounded hover:bg-red-500/10 text-gray-400 hover:text-red-400 transition-colors">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div x-show="isOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            ...
            <div class="relative min-h-screen flex items-center justify-center p-4">
                <div class="relative bg-gray-800/80 backdrop-blur-xl rounded-xl border border-gray-700/50 shadow-xl w-full max-w-md overflow-hidden">
                    ...
                    <form action="{{ Route('transaction.submit') }}" method="POST" class="p-5" x-data="{ transactionType: 'ponctuelle' }">
                        @csrf
                        <div class="space-y-4">
                            <!-- Type Selection -->
                            <div>
                            ...
                            <div x-show="transactionType === 'recurrentes'" x-transition>
                                ...
                                <input type="number" name="date" min="1" max="31" placeholder="1-31" x-bind:required="transactionType === 'recurrentes'"
                                    class="w-full bg-gray-700/40 border border-gray-600 rounded-lg py-2 px-3 text-white focus:border-blue-500 focus:ring focus:ring-blue-500/20">
                            </div>
                            
                            <!-- Submit Button -->
                            <div class="pt-2">
                                <button type="submit" class="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">
                                    Add Transaction
                                </button>
                            </div>
                        </div>
